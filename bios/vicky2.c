...
static const uint16_t tt_dflt_palette[] = {
 TTRGB_WHITE, /*TTRGB_RED*/TTRGB_BLACK/* problem with bitmap engine ? no contrast so for dbeug I use black*/, TTRGB_GREEN, TTRGB_YELLOW,
...
{    
    ...
    for (i = 0; i < 256; i++)
        convert_color(tt_dflt_palette[i],&(VICKY_LUT(0)[i]));
...
const uint8_t c4to8bits[] = {
    0x00, 0x11, 0x22, 0x33, 0x44, 0x44, 0x66, 0x77,
