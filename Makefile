...
# Override with 1 to use the m68k-atari-mintelf ELF toolchain.
# Unlike the default m68k-atari-mint one, it is recent enough to support LTO.
MINTELF = 0

ifeq (1,$(MINTELF))
ELF_TOOLCHAIN_PREFIX = m68k-atari-mintelf-
else
ELF_TOOLCHAIN_PREFIX = m68k-elf-
endif

ifneq (,$(filter 1,$(ELF) $(MINTELF)))
# Standard ELF toolchain
TOOLCHAIN_PREFIX = $(ELF_TOOLCHAIN_PREFIX)
...
# Declaration mismatches between translation units are only diagnosable once
# the whole program is visible, i.e. at LTO link time. They silently generate
# wrong code, so make them fatal.
LTOFLAGS = -flto=auto -Werror=lto-type-mismatch -Werror=odr
CFLAGS += $(LTOFLAGS)
LDFLAGS += $(LTOFLAGS) $(CFLAGS_COMPILE)
# The plain ar/nm/ranlib in a cross toolchain often pick up the *host* LTO
# plugin, which silently mangles the symbol index of an archive of LTO objects.
# The gcc-prefixed wrappers always pass the matching plugin.
AR = $(TOOLCHAIN_PREFIX)gcc-ar
NM = $(TOOLCHAIN_PREFIX)gcc-nm
endif

ifneq (,$(filter 1,$(ELF) $(MINTELF)))
# emutos.ld only places a known set of input sections. Should the compiler ever
# emit something else (.text.unlikely, .rodata.cst4, ...), the linker would
# silently append it as an orphan section outside of the ROM image.
LDFLAGS += -Wl,--orphan-handling=warn
endif

# The objdump utility (disassembler)
OBJDUMP = $(TOOLCHAIN_PREFIX)objdump

# The nm utility (symbol lister)
...
util_src = cookie.c doprintf.c intmath.c langs.c memmove.S memset.S miscasm.S \
	nls.c setjmp.S string.c lisautil.S miscutil.c shellutl.c
...
vdi_src = vdi_asm.S vdi_bezier.c vdi_col.c vdi_control.c vdi_esc.c \
          ...
          vdi_raster_driver.c \
          vdi_raster_bitplane.c vdi_raster_bitplane_line.c \
          vdi_raster_bitplane_pixel.c \
          vdi_raster_truecolor.c vdi_raster_chunky8.c \
		  mform.c \
...
a2560k: CPUFLAGS = -m68040 -msoft-float
...
a2560x: CPUFLAGS = -m68060 -msoft-float
...
#
# It is also necessary for files whose position inside the ROM matters: the
# linker inserts the single object generated by LTO at the position of the
# *first* LTO input, so every LTO'd file ends up grouped there. endrom.c must
# stay physically last, because ui_mupb has to be located above the AES entry
# point it points to (see the comment in bios/endrom.c).
NOLTO = bios/lowstram.c bios/endrom.c
...
#
# Static checks on the linked ROM
#
# BASELINE_MAP can point to the emutos.map of a reference build, to get a
# comparison of the code size and of the symbols which are no longer global.
#
...
.PHONY: check
NODEP += check
check: check_target_exists
	NM='$(NM)' OBJDUMP='$(OBJDUMP)' bash tools/checkrom.sh \
	  $(if $(BASELINE_MAP),-b $(BASELINE_MAP)) emutos.img emutos.map
...
check_target_exists:
	@test -f emutos.img -a -f emutos.map \
	  || (echo "Please make a target first." >&2 ; false)
