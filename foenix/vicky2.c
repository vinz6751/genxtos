...
struct vicky2_channel_t vicky2_channel_a = {
    (struct vicky2_t *)VICKY_A,
    (struct vicky2_bitmap_t *)0L,
    (struct vicky2_tile_t *)0L,
    (struct vicky2_collision_t *)0L,
    (struct vicky2_mouse_gfx_t *)(VICKY_A + 0x0400), // Mouse graphics
    (struct vicky2_mouse_control_t *)(VICKY_A + 0x0C00), // Mouse control
    (struct vicky2_sprites_control_t **)0L,
    (struct vicky2_bmp_palette_t *)0L,
    (struct vicky2_gamma_lut_memory_t *)0L,
    (struct vicky2_font_memory_t *)VICKY_FONT_A,
    (struct vicky2_text_memory_t *)VICKY_TEXT_A,
    vicky2_a_video_modes
};

struct vicky2_channel_t vicky2_channel_b = {
    (struct vicky2_t *)VICKY_B,
    (struct vicky2_bitmap_t *)(VICKY_B+0x100),
    (struct vicky2_tile_t *)(VICKY_B+0x200),
    (struct vicky2_collision_t *)(VICKY_B + 0x300),
    (struct vicky2_mouse_gfx_t *)(VICKY_B + 0x0400), // Mouse graphics
    (struct vicky2_mouse_control_t *)(VICKY_B + 0x0C00), // Mouse control
    (struct vicky2_sprites_control_t **)(VICKY_B + 0x1000),
    (struct vicky2_bmp_palette_t *)(VICKY_B + 0x2000),
    (struct vicky2_gamma_lut_memory_t *)(VICKY_B + 0x4000),
    (struct vicky2_font_memory_t *)VICKY_FONT_B,
    (struct vicky2_text_memory_t *)VICKY_TEXT_B,
...
// Default screen
struct vicky2_channel_t * const vicky = &vicky2_channel_b;
...
void vicky2_init_channel(struct vicky2_channel_t *vicky)
{
    ...
    uint16_t *w;
    const COLOR32 bg_color = {  /* Blue-ish background */
        .alpha = 0xff,
        .red = 0x0e,
        .green = 0x2b,
        .blue = 0x4f
    };
    a2560_debugnl("vicky2_init:1");
#if 0
//#ifdef MACHINE_A2560X
    // Initialise text screen for logging
    vicky_a->control = VICKY_CTRL_TEXT; // Default is 800x600
    vicky_a->background_color = 0xff8888ff;
    vicky_a->border_control = 0x00080800|VICKY_BORDER_ENABLE;
    vicky_a->border_color = 0xffffffff;
    vicky_a->background_color = 0x00222222;//convert_atari2vicky_color(vicky2_default_palette[15]);
    vicky_a->cursor_control = 0xf0ff0001;
    vicky_a->cursor_position = 0x00000000;
    channel_A_set_fg_color(0);
    channel_A_set_bg_color(15);
    channel_A_cls();
    for (i=0; i<16; i++) {
        vicky_text_a->palette_bg[i].code = vicky_text_a->palette_fg[i].code = convert_atari2vicky_color(vicky2_default_palette[i]);
    }
#endif
...
a2560_debugnl("vicky2_init:4");
    ...
    a2560_debugnl("vicky2_init:5");
        
        /* Clear the screen */
        fb_size = (uint32_t)mode.w * mode.h / sizeof(uint16_t);
        fb_size -= 1;
        w = (uint16_t*)VRAM_Bank0;
    a2560_debugnl("vicky2_init:6");
        while (fb_size--)
            *w++ = 0;
    }

a2560_debugnl("vicky2_init:7");
}
...
void vicky2_set_video_mode(struct vicky2_channel_t *vicky, uint16_t mode)
{
    ...
    a2560_debugnl("Video mode %d (%d,%d) set, new VICKY ctrl: %p", mode, vicky->video_modes[mode].w, vicky->video_modes[mode].h, (void*)vicky->ctrl->control);
}


void vicky2_set_bitmap_address(struct vicky2_channel_t *vicky, uint16_t layer,const uint8_t *address)
{
#ifdef MACHINE_A2560U
